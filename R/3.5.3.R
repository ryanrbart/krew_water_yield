...
#
# Contains scripts for evaluating CHANGE in response variables
...
x <- filter(p301_shed_diff, var_type == "streamflow") %>%
  ...
  NULL
print(x)
ggsave("output/3.5_simulations/precip_vs_change_in_q.jpg", plot=x, width = 6, height = 4)
...
x <- filter(p301_shed_diff, var_type == "et") %>%
  ...
  NULL
print(x)
ggsave("output/3.5_simulations/precip_vs_change_in_et.jpg", plot=x, width = 6, height = 4)
...
x <- filter(p301_shed_diff, var_type == "trans") %>%
  ...
  NULL
print(x)
ggsave("output/3.5_simulations/precip_vs_change_in_trans.jpg", plot=x, width = 6, height = 4)
...
x <- filter(p301_shed_diff, var_type == "evap") %>%
  ...
  NULL
print(x)
ggsave("output/3.5_simulations/precip_vs_change_in_evap.jpg", plot=x, width = 6, height = 4)
...
x <- filter(p301_shed_diff_avg, var_type == "rz_storage") %>%
  ...
  geom_point(aes(x=annual_precip,y=change, shape=as.character(thinning)), size = 1.5) +
  scale_shape_manual(values=seq(1,9), name="Percent\nForest\nThinned", labels=c(10,20,30,40,50,60,70,80,90)) +
  labs(x = "Annual Precipitation (mm)", y = "Change in Annual RZ Storage (mm)") +
  facet_grid(wy~soil_depth, labeller = labeller(.rows=post_fire_year_id, .cols=soil_depth_id)) +
  theme_bw() +
  NULL
print(x)
ggsave("output/3.5_simulations/precip_vs_change_in_rz_storage.jpg", plot=x, width = 6, height = 4)
...
x <- filter(p301_shed_diff_avg, var_type == "sat_def") %>%
  ...
  geom_point(aes(x=annual_precip,y=change, shape=as.character(thinning)), size = 1.5) +
  scale_shape_manual(values=seq(1,9), name="Percent\nForest\nThinned", labels=c(10,20,30,40,50,60,70,80,90)) +
  labs(x = "Annual Precipitation (mm)", y = "Change in Saturated Deficit (mm)") +
  facet_grid(wy~soil_depth, labeller = labeller(.rows=post_fire_year_id, .cols=soil_depth_id)) +
  theme_bw() +
  NULL
print(x)
ggsave("output/3.5_simulations/precip_vs_change_in_sat_def.jpg", plot=x, width = 6, height = 4)
...
x <- filter(p301_shed_diff_avg, var_type == "unsat_stor") %>%
  ...
  geom_point(aes(x=annual_precip,y=change, shape=as.character(thinning)), size = 1.5) +
  scale_shape_manual(values=seq(1,9), name="Percent\nForest\nThinned", labels=c(10,20,30,40,50,60,70,80,90)) +
  labs(x = "Annual Precipitation (mm)", y = "Change in Unsaturated Storage (mm)") +
  facet_grid(wy~soil_depth, labeller = labeller(.rows=post_fire_year_id, .cols=soil_depth_id)) +
  theme_bw() +
  NULL
print(x)
ggsave("output/3.5_simulations/precip_vs_change_in_unsat_stor.jpg", plot=x, width = 6, height = 4)
...
x <- filter(p301_shed_diff, var_type == "height") %>%
  ...
  NULL
print(x)
ggsave("output/3.5_simulations/precip_vs_change_in_height.jpg", plot=x, width = 6, height = 4)
