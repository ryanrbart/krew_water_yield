...
# (Similar to 2.3_paired_mixed_model_runs_reverse_control.R, but with interaction variable)
...
out_wyb_int_alt <- pair_wy_alt %>% 
  #dplyr::filter(shed_treated %in% c("P301","D102","P304")) %>% 
  dplyr::filter(shed_treated %in% c("B201","B203","B204")) %>% 
  paired_function1(DATA=., ITER=ITER, WARMUP=WARMUP, CHAINS=CHAINS,
                   CORES=CORES, THIN=THIN, SEED=SEED, ADAPT_DELTA=ADAPT_DELTA)

out_wyp_int_alt <- pair_wy_alt %>% 
  dplyr::filter(shed_treated %in% c("P301","D102","P304")) %>% 
  #dplyr::filter(shed_treated %in% c("B201","B203","B204")) %>% 
  paired_function1(DATA=., ITER=ITER, WARMUP=WARMUP, CHAINS=CHAINS,
                   CORES=CORES, THIN=THIN, SEED=SEED, ADAPT_DELTA=ADAPT_DELTA)
...
summary(out_wy_int_alt, digits=2, probs = c(0.025, 0.975))
summary(out_wyb_int_alt, digits=2, probs = c(0.025, 0.975))
summary(out_wyp_int_alt, digits=2, probs = c(0.025, 0.975))

out_q_int_alt <- list(out_wy_int_alt,
                      out_wyb_int_alt,
                      out_wyp_int_alt)
...
out_q_int_alt_draws <- generate_draws(out_q_int_alt)


out_q_int_alt_draws <- out_q_int_alt_draws %>% 
  dplyr::mutate(pair_type = factor(case_when(response_variable == 1 ~ "All",
                                       response_variable == 2 ~ "Bull",
                                       response_variable == 3 ~ "Providence"),
                                   levels=c("Providence", "Bull", "All")))


# ------------------------------------------------------------
# Plot the draws for the NDVI and interaction variable


watershed_id <- c(All = "All\nWatersheds",
                  Bull = "Bull",
...
x <- out_q_int_alt_draws %>%      
  ggplot(data=., aes(y = pair_type, x = ndvi_diff_n)) +
  ...
  xlim(-10,10) +
  NULL
ggsave("output/2.4_mixed_model_analysis/plot_mixed_model_rev_control_int_beta1.jpg",plot=x, width = 5, height = 3)
#plot(x)


# Plot uncertainty intervals by parameter (interaction variable for Bull, Prov1 and Prov2)
x <- out_q_int_alt_draws %>%      
  ggplot(data=., aes(y = pair_type, x = `log(q_control):ndvi_diff_n`)) +
  tidybayes::geom_halfeyeh(.width = c(0.9, 0.95)) +
  geom_vline(xintercept = 0) +
  scale_y_discrete(labels = c(watershed_id)) +
  ...
  xlim(-1.75,1.75) +
  NULL
ggsave("output/2.4_mixed_model_analysis/plot_mixed_model_rev_control_int_beta2.jpg",plot=x, width = 5, height = 3)
#plot(x)
